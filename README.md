...
- **豊富な対応フォーマット** — 音声データ（wav, mp3, m4a, flac, ogg, opus, wma, aiff, aif, alac等）、映像データ（mp4, mov, mkv, m4v, avi等）に対応。別途インストールしたFFmpegを利用します。
...
リリースはタグベースで実行されます。例: `v26.7.12`。

```bash
# 下書き確認
pnpm release --dry-run v26.7.12

# 実行
pnpm release v26.7.12
```

実行時に次を自動実施します。

- `package.json`, `src-tauri/Cargo.toml`, `src-tauri/Cargo.lock`, `src-tauri/tauri.conf.json` の version 更新（`26.7.12`）
- リリースコミットを作成して `v26.7.12` のタグを作成・Push
- 該当タグの GitHub Actions（`build.yml`）の完了待機
- GitHub Releases の自動公開（GitHub側で `release` ジョブが実行される）

補助として、次コマンドでリリースノート本文（概要＋変更履歴＋配布ファイル）の下書きを生成できます。

```bash
pnpm release:notes v26.7.12
```

公開後は `github-release` スキルを使い、過去のリリース形式とコミット内容を踏まえて本文を日本語で整え、GitHub Releases に直接反映します。
...
Copyright © 2026 NJSoftware

### 依存ライブラリとライセンス

複数のライセンスを選べる依存ライブラリは、MITライセンスを選択しています。完全な第三者ライセンス通知は、アプリの「バージョン情報」から確認できます。
...
| tokio | 1.52 | MIT |
| serde / serde_json | 1.0 | MIT |
| anyhow / walkdir / uuid | 1.0 / 2.5 / 1.23 | MIT |
| dirs / base64 / sysinfo | 6.0 / 0.22 / 0.32 | MIT |
| libc / windows-sys | 0.2 / 0.59 | MIT |
| FFmpeg（別途インストール） | — | 配布物による（LGPL 2.1+ または GPL） |
...
- **Wide range of supported formats** — Supports audio data (wav, mp3, m4a, flac, ogg, opus, wma, aiff, aif, alac, etc.) and video data (mp4, mov, mkv, m4v, avi, etc.) through a separately installed FFmpeg.
...
Where a dependency offers multiple licenses, oTo selects MIT. The complete third-party notices are available from the app's About window.
...
| dirs / base64 / sysinfo | 6.0 / 0.22 / 0.32 | MIT |
| libc / windows-sys | 0.2 / 0.59 | MIT |
| FFmpeg (installed separately) | — | Depends on the distribution (LGPL 2.1+ or GPL) |
