...
macOS / Windows 向けのシンプルで高速なオーディオ変換アプリ。FFmpeg をサブプロセスとして呼び出し、ドラッグ＆ドロップによりファイルを選択して、多様なオーディオフォーマットへの変換を行います。
...
![Main Window](img/main_window1.png)
![Main Window](img/main_window2.png)
...
- **豊富な対応フォーマット** — wav, mp3, m4a, flac, mp4, ogg, opus, wma, aiff, aif, alac
- **エンコード / デコード** — オーディオ/ビデオからオーディオへのエンコードと、復号（デコード）モードの両方に対応
...
### OGG フォーマットのエンコードについて

OGG (Vorbis) へのエンコードには、FFmpeg が **libvorbis** を含むビルドである必要があります。  
macOS の Homebrew 版 FFmpeg は通常 libvorbis を含みますが、含まれていない場合は以下を実行してください：

```bash
brew reinstall ffmpeg
```

アプリ内で OGG エンコード時に libvorbis が見つからない旨のエラーが表示された場合も、上記コマンドで解決できます。
