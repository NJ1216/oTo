...
function populateForm(s) {
  ...
  document.getElementById('autoOpenActivity').checked = s.autoOpenActivity ?? false;
  document.getElementById('exportAllAudioTracks').checked = s.exportAllAudioTracks ?? false;
...
function collectFormValues() {
  ...
  return {
    ...
    autoOpenActivity: chk('autoOpenActivity'),
    exportAllAudioTracks: chk('exportAllAudioTracks'),
...
function scheduleAutoSave() {
  ...
  const preview = { ...settings, ...collectFormValues() };
  settings = preview;
  emit('settings_preview_updated', preview).catch(() => {});
  _saveTimer = setTimeout(async () => {
    try {
      const latest = await invoke('get_settings');
      const saved = { ...latest, ...collectFormValues() };
      await invoke('save_settings', { s: saved });
      settings = saved;
      await emit('settings_updated', saved);
    } catch (_) {}
  }, 300);
}

// 全 input / select の change で自動保存
